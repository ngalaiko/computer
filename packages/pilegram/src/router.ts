...
export class Router {
  ...
  private async build(
    ...
    const resourceLoader = new DefaultResourceLoader({
      ...
      extensionFactories: [
        contextExtension({
          ...
          isTopic,
          topicName: () =>
            this.db.getRoute(chatId, threadId)?.name ?? undefined,
          topicIcon: () =>
            this.db.getRoute(chatId, threadId)?.icon ?? undefined,
        }),
      ],
    });
    ...
    const session = await Session.create({
      ...
      customTools: buildRouteTools({
        ...
        deleteTopic: (topicThreadId) =>
          this.deleteTopic(route.chatId, topicThreadId),
        setTopic: (o) =>
          this.renameTopic(route, { name: o.name, iconEmoji: o.icon }),
      }),
      onFinalized: (text) => {
        if (text && text.trim() !== "")
          this.db.setRouteLastRenderedSig(chatId, threadId, sig(text));
      },
    });

    if (!reopen) {
      this.db.setRouteSessionFile(
        chatId,
        threadId,
    ...
    if (reopen)
      this.reconcile(route, session, prior?.lastRenderedSig ?? null, writer);
  ...
  private reconcile(
    ...
    writer
      .persist(`↻ reconnected\n\n${last}`)
      .catch((e) => log.error("reconcile persist failed", errFields(e)));
    this.db.setRouteLastRenderedSig(
      route.chatId,
      route.threadId ?? 0,
      sig(last),
    );
    ...
    if (!this.iconSet) {
      ...
      try {
        for (const s of await this.api.getForumTopicIconStickers()) {
          if (s.custom_emoji_id && s.emoji && !byEmoji.has(s.emoji))
            byEmoji.set(s.emoji, s.custom_emoji_id);
        }
      } catch (e) {
  ...
  async createTopic(
    ...
    const icon_custom_emoji_id = opts.icon
      ? await this.iconId(opts.icon)
      : undefined;
    const topic = await this.api.createForumTopic(
      chatId,
      name,
      icon_custom_emoji_id ? { icon_custom_emoji_id } : undefined,
    );
  ...
  async deleteTopic(chatId: number, threadId: number): Promise<void> {
    if (!Number.isSafeInteger(threadId) || threadId <= 0)
      throw new Error("provide a valid non-General topic thread id");
    try {
      await this.api.deleteForumTopic(chatId, threadId);
    } catch (e) {
      if (!isMissingTopic(e)) throw e;
      log.warn("topic already missing while deleting; marking route ended", {
        chatId,
        threadId,
        ...errFields(e),
      });
  ...
  async renameTopic(
    route: Route,
    opts: { name?: string; iconEmoji?: string },
  ): Promise<void> {
    const threadId = route.threadId;
    if (threadId === undefined || threadId === 0)
      throw new Error("the General thread has no topic to rename");
    const edit: { name?: string; icon_custom_emoji_id?: string } = {};
    if (opts.name) edit.name = opts.name;
    if (opts.iconEmoji)
      edit.icon_custom_emoji_id = await this.iconId(opts.iconEmoji);
    if (edit.name === undefined && edit.icon_custom_emoji_id === undefined)
      return;
    ...
    if (opts.iconEmoji)
      this.db.setRouteIcon(route.chatId, threadId, opts.iconEmoji);
  }
...
function cleanTopicName(raw: string): string {
  ...
  if (name.length < 1 || name.length > 128)
    throw new Error("topic name must be 1-128 characters");
  return name;
}
...
function isMissingTopic(e: unknown): boolean {
  ...
  return /TOPIC_ID_INVALID|thread|topic|message thread/i.test(
    e.description ?? "",
  );
}
