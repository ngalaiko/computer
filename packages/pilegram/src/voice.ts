...
async function run(cmd: string[], stdin?: Uint8Array): Promise<string> {
  ...
  if (code !== 0)
    throw new Error(`${cmd[0]} exited ${code}: ${stderr.slice(0, 500)}`);
  return stdout;
}
...
export class Voice {
  ...
  constructor(private readonly cfg: VoiceConfig) {
    ...
    this.voiceStylePath = join(
      cfg.modelsDir,
      "supertonic",
      "voice_styles",
      `${cfg.supertonicVoice}.json`,
    );
  }
  ...
  private async ensureStt() {
    if (!existsSync(this.whisperModelPath)) {
      await download(
        `${WHISPER_BASE}/ggml-${this.cfg.whisperModel}.bin`,
  ...
  private async ensureTtsAssets() {
    ...
    for (const f of SUPERTONIC_ONNX) {
      ...
      if (!existsSync(dest))
        await download(`${SUPERTONIC_BASE}/onnx/${f}`, dest);
    }
    if (!existsSync(this.voiceStylePath)) {
      await download(
        `${SUPERTONIC_BASE}/voice_styles/${this.cfg.supertonicVoice}.json`,
        this.voiceStylePath,
      );
    }
  }
    ...
    if (!this.ttsPromise) {
      this.ttsPromise = (async () => {
        ...
        log.info("supertonic ready", {
          voice: this.cfg.supertonicVoice,
