...
export class Gateway {
  ...
  private readonly albums = new Map<
    string,
    {
      route: Route;
      msgs: Message[];
  ...
  async start() {
    ...
    if (!topicsEnabled) {
      ...
      log.info(
        "topic mode unavailable — running DM single-session mode (plan §2.4)",
      );
    }
    ...
    while (this.running) {
      ...
      try {
        updates = await this.bot.api.getUpdates(
          ...
          this.abort.signal as unknown as Parameters<
            Bot["api"]["getUpdates"]
          >[1],
        );
      } catch (e) {
        ...
        if (e instanceof GrammyError && e.error_code === 409) {
          ...
          log.error(
            "409 Conflict: another getUpdates poller is running — shutting down",
            errFields(e),
          );
          this.running = false;
          throw e;
        ...
        log.warn("getUpdates failed — retrying", {
          ...errFields(e),
          http: e instanceof HttpError,
        });
        await sleep(1000);
        continue;
      }
  ...
  private async dispatch(update: Update) {
    ...
    if (msg.forum_topic_edited) {
      if (msg.forum_topic_edited.name)
        this.router.setName(route, msg.forum_topic_edited.name);
      return;
    }
    ...
    if (typeof msg.text === "string") {
      ...
      await session.handlePrompt(prefix + msg.text, {
    ...
    if (msg.photo) {
      ...
      await this.guardMedia(session, async () => {
        ...
        await session.handlePrompt(prefix + (msg.caption ?? "(image)"), {
          images: [img],
          messageId: msg.message_id,
        });
      });
    ...
    if (msg.document) {
      const session = await this.router.getOrCreate(route);
      await this.guardMedia(session, () =>
        this.ingestDocument(session, msg, prefix),
      );
      return;
    }
    ...
    if (msg.sticker) {
      ...
      await session.handlePrompt(
        `${prefix}[sticker: ${msg.sticker.emoji ?? ""}]`,
        { messageId: msg.message_id },
      );
      return;
    }
    ...
    if (msg.voice || msg.video_note || msg.audio) {
      ...
      await this.guardMedia(session, async () => {
        ...
        await session.handlePrompt(prefix + transcript, {
          messageId: msg.message_id,
          speak: true,
        });
      });
    ...
    if (descriptor) {
      const session = await this.router.getOrCreate(route);
      await session.handlePrompt(prefix + descriptor, {
        messageId: msg.message_id,
      });
      return;
    }
    ...
    if (msg.video) {
      ...
      await session.notify(
        "(send a voice note if you want me to listen; plain video isn't handled yet.)",
      );
      return;
    }
  }
  ...
  private async photoToImage(
    ...
    const dl = await downloadTelegramFile(
      ...
      this.config.botToken,
      largest.file_id,
      join(session.workspaceDir, "inbox"),
      {
        mime: "image/jpeg",
      },
    );
  ...
  private async ingestDocument(session: Session, msg: Message, prefix = "") {
    ...
    let text =
      prefix +
      (msg.caption?.trim() ||
        `I've saved a file you sent to ${dl.path}. Take a look.`);
    if (msg.caption) text += `\n\n(saved to ${dl.path})`;
  ...
  private async flushAlbum(gid: string) {
    ...
    await this.guardMedia(session, async () => {
      ...
      log.info("album flush", {
        items: pending.msgs.length,
  ...
  private async handleCallback(cq: CallbackQuery) {
    ...
    if (action === "t") {
      ...
      await this.answerCallback(
        cq.id,
        entry.selected.has(index) ? "Added" : "Removed",
      );
      await this.editKeyboard(
        cq,
        renderQuestionKeyboard(id, entry.options, entry.selected, true),
      );
      return;
    }
    ...
    const indices =
      action === "d"
        ? [...(this.questions.get(id)?.selected ?? [])].sort((a, b) => a - b)
        : Number.isNaN(index)
          ? []
          : [index];
    ...
    await this.answerCallback(
      cq.id,
      chosen ? "Got it" : "This choice has expired.",
    );
    const label = (
      chosen
        ? chosen.length
  ...
  private logReaction(mr: MessageReactionUpdated) {
    const emojis = mr.new_reaction.map((r) =>
      r.type === "emoji" ? r.emoji : r.type,
    );
...
function forwardDescriptor(msg: Message): string | undefined {
  ...
  const title = (c: { title?: string; first_name?: string }) =>
    c.title ?? c.first_name ?? "a chat";
  switch (f.type) {
    case "user":
...
function isTextual(mime: string, name: string): boolean {
  ...
  if (
    ...
    return true;
  return /\.(txt|md|markdown|json|ya?ml|csv|tsv|log|ts|tsx|js|jsx|py|rs|go|java|c|h|cpp|sh|bash|zsh|toml|ini|cfg|conf|xml|html?|css|sql)$/i.test(
    name,
  );
}
