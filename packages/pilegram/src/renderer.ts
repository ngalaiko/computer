...
export class Renderer {
  ...
  private preview?: {
    id?: number;
    starting?: Promise<{ message_id: number }>;
    latestText: string;
  };
  ...
  private sendFinal(chunk: string, extra: { parse_mode: "HTML" } | undefined) {
    this.writer
      ...
      .catch((e) => {
        ...
        this.log.warn(
          "HTML finalize rejected; retrying as plain text",
          errFields(e),
        );
        ...
        this.writer
          ...
          .catch((e2) =>
            this.log.error("plain finalize fallback failed", errFields(e2)),
          );
      });
  }
  ...
  onError(err: unknown) {
    ...
    this.writer
      .persist(`⚠️ ${msg}`)
      .catch((e) => this.log.error("error persist failed", errFields(e)));
  }
  ...
  private flushNow() {
    ...
    preview.starting = this.writer
      ...
      .then(async (message) => {
        ...
        if (preview.latestText !== initialText)
          await this.writer.editText(message.message_id, preview.latestText);
        return message;
      })
      .catch((e) => {
  ...
  private async finalizePreview(
    ...
    chunks: string[],
    extra: { parse_mode: "HTML" } | undefined,
  ) {
    if (!preview) {
    ...
    try {
      ...
      const id = preview.id ?? message?.message_id;
      if (id === undefined)
        throw new Error("preview did not yield a message id");
      ...
      this.log.warn(
        "preview finalize failed; sending standalone answer",
        errFields(e),
      );
      for (const chunk of chunks) this.sendFinal(chunk, extra);
    }
...
function summarizeArgs(args: unknown): string {
  ...
  const pick =
    a.command ?? a.cmd ?? a.path ?? a.file_path ?? a.pattern ?? a.query;
  const s = typeof pick === "string" ? pick : JSON.stringify(args);
  const flat = s.replace(/\s+/g, " ").trim();
  return flat === ""
    ? ""
    : ` · ${flat.length > 80 ? flat.slice(0, 79) + "…" : flat}`;
}
...
function formatToolSummary(
  ...
  const total = [...counts.values()].reduce((a, b) => a + b, 0);
  const parts = [...counts.entries()].map(([name, c]) =>
    c > 1 ? `${name} ×${c}` : name,
  );
