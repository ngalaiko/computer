...
function renderInline(tokens: Token[] | undefined, raw: string): string {
  ...
  for (const t of tokens as Tokens.Generic[]) {
    switch (t.type) {
      case "text":
        out += t.tokens
          ? renderInline(t.tokens, t.raw)
          : esc((t as Tokens.Text).text ?? t.raw);
        break;
      case "escape":
...
function renderList(t: Tokens.List): string {
  ...
  t.items.forEach((item, i) => {
    ...
    const body = renderInline(item.tokens, item.text)
      .replace(/\n+/g, " ")
      .trim();
...
function renderBlock(t: Tokens.Generic): string {
  switch (t.type) {
    ...
    case "code": {
      ...
      const cls = c.lang
        ? ` class="language-${escAttr(c.lang.split(/\s+/)[0] ?? "")}"`
        : "";
      return `<pre><code${cls}>${esc(c.text)}</code></pre>`;
    }
    case "blockquote": {
      const inner = (t.tokens as Tokens.Generic[])
        .map(renderBlock)
        .filter(Boolean)
        .join("\n");
...
export function renderMarkdownBlocks(md: string): string[] {
  ...
  return tokens
    .map((t) => renderBlock(t as Tokens.Generic).trim())
    .filter((s) => s.length > 0);
}
