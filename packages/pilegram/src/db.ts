...
export class Db {
  ...
  getRoute(chatId: number, threadId: number): RouteRow | undefined {
    const row = this.db
      .query("SELECT * FROM routes WHERE chat_id = ? AND thread_id = ?")
      .get(chatId, threadId) as RouteDbRow | null;
  ...
  private updateRoute(
    ...
    this.db
      .query(
        `UPDATE routes SET ${column} = ?, updated_at = ? WHERE chat_id = ? AND thread_id = ?`,
      )
  ...
  getScheduledTask(id: string): ScheduledTaskRow | undefined {
    const row = this.db
      ...
      .get(id) as ScheduledTaskDbRow | null;
  ...
  listScheduledTasks(): ScheduledTaskRow[] {
    const rows = this.db
      .query(
        "SELECT * FROM scheduled_tasks ORDER BY enabled DESC, title COLLATE NOCASE",
      )
      .all() as ScheduledTaskDbRow[];
  ...
  setScheduledTaskRoute(id: string, chatId: number, threadId: number) {
    this.db
      .query(
        "UPDATE scheduled_tasks SET task_chat_id = ?, task_thread_id = ?, updated_at = ? WHERE id = ?",
      )
  ...
  getMediaFileId(sha256: string): string | undefined {
    const row = this.db
      .query("SELECT file_id FROM media WHERE sha256 = ?")
      .get(sha256) as { file_id: string } | null;
    return row?.file_id;
  }
