...
export class Scheduler {
  ...
  update(params: ScheduleUpdateParams): ScheduledTaskRow {
    ...
    if (current.kind !== "recurring")
      throw new Error(`not a recurring scheduled task: ${params.id}`);
    ...
    if (title !== current.title) {
      void this.router
        .renameTopic(
          { chatId: updated.taskChatId, threadId: updated.taskThreadId },
          { name: `📆 ${title}` },
        )
        .catch((e) =>
          this.log.warn("failed to rename scheduled-task topic", {
            id: params.id,
            ...errFields(e),
          }),
        );
  ...
  private async tick() {
    ...
    try {
      ...
      for (const task of due)
        await this.runTask(task).catch((e) =>
  ...
  private async runTask(task: ScheduledTaskRow) {
    ...
    const nextRunAt =
      task.kind === "recurring"
        ? nextCronRun(task.cron, task.timezone, new Date(now))
        : now;
    ...
    try {
      ...
      this.log.warn("scheduled-task topic is missing; recreating", {
        id: task.id,
        title: task.title,
        ...errFields(e),
      });
  ...
  private async dispatch(task: ScheduledTaskRow) {
    ...
    const session = await this.router.getOrCreate(route, {
      name: task.kind === "recurring" ? `📆 ${task.title}` : undefined,
    });
    ...
    const label =
      task.kind === "recurring" ? "scheduled task" : "one-off notification";
    await session.handlePrompt(`[${label}: ${task.title}]\n${task.prompt}`);
  }

  private async recreateTaskTopic(
    task: ScheduledTaskRow,
  ): Promise<ScheduledTaskRow> {
    if (task.kind !== "recurring")
      throw new Error("one-off notification target thread is missing");
...
function notificationRunAt(params: NotificationCreateParams): number {
  ...
  if (!Number.isFinite(t))
    throw new Error("runAt must be an absolute RFC3339/ISO timestamp");
