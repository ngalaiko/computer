...
function askQuestion(
  ...
  return new Promise<string[]>((resolve, reject) => {
    ...
    const id = ctx.questions.create(options, opts.multi, (indices) => {
      finish(() =>
        indices === null
          ? reject(new Error("question dismissed"))
    ...
    ctx.writer
      .persist(stripUnsafe(question), { reply_markup: keyboard })
      .catch((e) => {
        finish(() => {
          ctx.questions.cancel(id);
...
export function buildRouteTools(ctx: RouteToolContext): ToolDefinition[] {
  const sendPhoto = defineTool({
    ...
    promptSnippet:
      "tg_send_photo({path, caption?, spoiler?}) — send an image to the user",
    parameters: Type.Object({
      path: Type.String({
        description:
          "Path to the image (absolute, or relative to the working directory).",
      }),
      caption: Type.Optional(Type.String({ description: "Optional caption." })),
      spoiler: Type.Optional(
        Type.Boolean({ description: "Hide behind a spoiler until tapped." }),
      ),
    }),
    async execute(_id, params) {
      ...
      if (bytes.byteLength > PHOTO_MAX)
        throw new Error(
          "image exceeds Telegram's 10MB photo cap — use tg_send_document",
        );
      ...
      const msg = cached
        ...
        : await ctx.writer.sendPhoto(
            new InputFile(bytes, basename(path)),
            opts,
          );
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Sent photo (message_id ${msg.message_id}).`,
          },
        ],
        details: {},
      };
  ...
  const sendDocument = defineTool({
    ...
    label: "Send Document",
    description:
      "Send a file to the user in this Telegram chat (any type). Use for logs, generated files, or large images.",
    promptSnippet:
      "tg_send_document({path, caption?}) — send a file to the user",
    parameters: Type.Object({
      path: Type.String({
        description:
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Sent ${basename(path)} (message_id ${msg.message_id}).`,
          },
        ],
        details: {},
      };
  ...
  const ask = defineTool({
    ...
    promptSnippet:
      "tg_ask({question, options[], multiSelect?}) — ask the user to pick option(s)",
    parameters: Type.Object({
      question: Type.String(),
      options: Type.Array(Type.String(), { minItems: 2, maxItems: 8 }),
      multiSelect: Type.Optional(
        Type.Boolean({
          description: "Allow picking multiple options (tick + Done).",
        }),
      ),
      timeoutSec: Type.Optional(
        Type.Number({ description: "Auto-cancel after this many seconds." }),
      ),
    }),
    async execute(_id, params, signal) {
      const chosen = await askQuestion(
        ctx,
        params.question,
        params.options,
        { multi: params.multiSelect ?? false, timeoutSec: params.timeoutSec },
        signal,
      );
  ...
  const confirm = defineTool({
    ...
    label: "Confirm",
    description:
      'Ask the user a yes/no question with tappable Yes/No buttons and wait. Returns "Yes" or "No".',
    ...
    async execute(_id, params, signal) {
      const chosen = await askQuestion(
        ...
        params.question,
        ["Yes", "No"],
        { multi: false, timeoutSec: params.timeoutSec },
        signal,
      );
      return {
        content: [{ type: "text" as const, text: chosen[0] ?? "No" }],
        details: {},
      };
    },
  });
  ...
  const react = defineTool({
    ...
    promptSnippet:
      "tg_react({emoji, messageId?}) — react to a message instead of replying in text",
    parameters: Type.Object({
      emoji: Type.String({
        description: "A single emoji from Telegram's reaction set.",
      }),
      messageId: Type.Optional(
        Type.Number({
          description: "Message to react to; defaults to the current message.",
        }),
      ),
    }),
    async execute(_id, params) {
      ...
      return {
        content: [{ type: "text" as const, text: `Reacted ${params.emoji}.` }],
        details: {},
      };
    },
  });
  ...
  const sendAlbum = defineTool({
    ...
    promptSnippet:
      "tg_send_album({paths[], caption?}) — send several images/files as one album",
    parameters: Type.Object({
    ...
    async execute(_id, params) {
      ...
      const allImages = items.every((it) =>
        /\.(jpe?g|png|gif|webp)$/i.test(it.path),
      );
      const file = (it: { path: string; bytes: Buffer }) =>
        new InputFile(it.bytes, basename(it.path));
      const cap = (i: number) =>
        i === 0 && params.caption ? { caption: params.caption } : {};
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Sent album of ${msgs.length} (message_ids: ${msgs.map((m) => m.message_id).join(", ")}).`,
          },
        ],
        details: {},
      };
  ...
  const sendLocation = defineTool({
    ...
    label: "Send Location",
    description:
      "Share a location on the map. If title and address are given, sends a named venue; otherwise a plain pin.",
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Sent location (message_id ${msg.message_id}).`,
          },
        ],
        details: {},
      };
  ...
  const pin = defineTool({
    ...
    label: "Pin Note",
    description:
      "Post a short note and pin it in this chat/topic so it stays visible (e.g. a summary or key result). Use tg_unpin to clear pins.",
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Pinned (message_id ${msg.message_id}).`,
          },
        ],
        details: {},
      };
  ...
  const unpin = defineTool({
    ...
    async execute() {
      ...
      return {
        content: [{ type: "text" as const, text: "Unpinned all messages." }],
        details: {},
      };
    },
  });
  ...
  const editMessage = defineTool({
    ...
    label: "Edit Message",
    description:
      "Edit the text of a message you previously sent, by its message_id (returned by the send tools).",
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Edited message ${params.messageId}.`,
          },
        ],
        details: {},
      };
  ...
  const deleteMessage = defineTool({
    ...
    description:
      "Delete a message by its message_id (one you sent, or the user's current message). Telegram only allows deleting messages under 48h old.",
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Deleted message ${params.messageId}.`,
          },
        ],
        details: {},
      };
  ...
  const createTopic = defineTool({
    ...
    promptSnippet:
      "tg_create_topic({name, icon?}) — create a Telegram topic/thread in this chat",
    parameters: Type.Object({
      name: Type.String({ description: "New topic title (1-128 characters)." }),
      icon: Type.Optional(
        Type.String({ description: "One allowed topic-icon emoji." }),
      ),
    }),
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Created topic \"${params.name}\" (thread_id ${route.threadId}).`,
          },
        ],
        details: {},
      };
  ...
  const listTopics = defineTool({
    ...
    promptSnippet:
      "tg_list_topics() — list known Telegram topics/threads in this chat",
    parameters: Type.Object({}),
    async execute() {
      ...
      const lines = routes.map((r) =>
        formatTopicRoute(r, scheduled.get(r.threadId) ?? []),
      );
      return {
        content: [
          {
  ...
  const deleteTopic = defineTool({
    ...
    promptSnippet:
      "tg_delete_topic({threadId}) — permanently delete a confirmed Telegram topic",
    parameters: Type.Object({
      threadId: Type.Number({
        description: "The topic's message_thread_id to delete.",
      }),
    }),
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Deleted topic thread_id ${params.threadId}.`,
          },
        ],
        details: {},
      };
  ...
  const setTopic = defineTool({
    ...
    promptSnippet:
      "tg_set_topic({name?, icon?}) — set the topic's title and/or icon",
    parameters: Type.Object({
      name: Type.Optional(
        Type.String({ description: "New topic title (2-5 words)." }),
      ),
      icon: Type.Optional(
        Type.String({ description: "One of the allowed topic icons." }),
      ),
    }),
    async execute(_id, params) {
      if (!params.name && !params.icon)
        throw new Error("provide name and/or icon");
      ...
      const changed = [
        ...
        .join(", ");
      return {
        content: [{ type: "text" as const, text: `Updated topic ${changed}.` }],
        details: {},
      };
    },
  });
  ...
  const scheduleCreate = defineTool({
    ...
    promptSnippet:
      "schedule_create({title, cron, prompt, timezone?}) — create a recurring task with its own thread",
    parameters: Type.Object({
      title: Type.String({
        description: "Short human title for the task/thread.",
      }),
      cron: Type.String({
        description:
      ...
      prompt: Type.String({
        description: "Prompt to run at each scheduled time.",
      }),
      timezone: Type.Optional(
        Type.String({
          description: "IANA time zone. Defaults to the gateway timezone.",
        }),
      ),
    }),
    async execute(_id, params) {
  ...
  const scheduleUpdate = defineTool({
    ...
    description:
      "Update a scheduled task's title, cron, timezone, prompt, or enabled state.",
    promptSnippet:
      "schedule_update({id, title?, cron?, prompt?, timezone?, enabled?}) — update a task",
    parameters: Type.Object({
      id: Type.String(),
      title: Type.Optional(Type.String()),
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: formatTask(ctx.scheduler.update(params)),
          },
        ],
        details: {},
      };
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: formatTask(ctx.scheduler.pause(params.id)),
          },
        ],
        details: {},
      };
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: formatTask(ctx.scheduler.resume(params.id)),
          },
        ],
        details: {},
      };
  ...
  const scheduleDelete = defineTool({
    ...
    description:
      "Delete a scheduled task. Its existing Telegram topic is left in place for history.",
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Deleted scheduled task ${params.id}.`,
          },
        ],
        details: {},
      };
  ...
  const notificationCreate = defineTool({
    ...
    promptSnippet:
      "notification_create({title, prompt, runAt? | delaySeconds?}) — create a one-off notification",
    parameters: Type.Object({
      title: Type.String({ description: "Short title for the notification." }),
      prompt: Type.String({
        description: "Prompt/message to deliver when the notification fires.",
      }),
      runAt: Type.Optional(
        Type.String({
          description:
            "Absolute RFC3339/ISO timestamp, e.g. 2026-08-08T15:30:00+02:00.",
        }),
      ),
      delaySeconds: Type.Optional(
        Type.Number({ description: "Delay from now in seconds." }),
      ),
    }),
    async execute(_id, params) {
      if (!ctx.scheduler) throw new Error("scheduler is not available");
    ...
    async execute(_id, params) {
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Cancelled notification ${params.id}.`,
          },
        ],
        details: {},
      };
  ...
  if (ctx.voice) {
    ...
    tools.push(
      defineTool({
        ...
        promptSnippet:
          "tg_send_voice({text?|path?}) — send a spoken voice note",
        parameters: Type.Object({
          text: Type.Optional(Type.String({ description: "Text to speak." })),
          path: Type.Optional(
            Type.String({
              description:
                "Path to an existing audio file (absolute or relative to the working directory).",
            }),
          ),
        }),
        async execute(_id, params) {
          if (params.path) {
            const p = isAbsolute(params.path)
              ? params.path
              : join(ctx.workspaceDir, params.path);
            if (!existsSync(p))
            ...
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Sent voice note (${basename(p)}).`,
                },
              ],
              details: {},
            };
          ...
          if (params.text) {
            const { path, durationSec } = await voice.synthesize(
              params.text,
              join(ctx.workspaceDir, "tts"),
            );
            await ctx.writer.sendVoice(new InputFile(path), {
            ...
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Sent voice note (${durationSec}s).`,
                },
              ],
              details: {},
            };
...
function formatTopicRoute(route: RouteRow, scheduledTitles: string[]): string {
  ...
  return [
    ...
    scheduledTitles.length
      ? `scheduled_tasks: ${scheduledTitles.join(", ")}`
      : undefined,
  ]
