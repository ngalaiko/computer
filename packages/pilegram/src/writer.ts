...
export class Writer {
  ...
  constructor(
    ...
    this.log = rootLog.child({
      route: `${route.chatId}-${route.threadId ?? 0}`,
    });
  }

  private get thread(): { message_thread_id?: number } {
    return this.route.threadId !== undefined
      ? { message_thread_id: this.route.threadId }
      : {};
  }
      ...
      try {
        ...
        if (e instanceof GrammyError && e.error_code === 429) {
          ...
          if (attempt > MAX_429_RETRIES) {
            this.log.error("429 retry budget exhausted — dropping op", {
              label,
              attempt,
              retryAfter,
            });
          ...
          this.log.warn("429 — honoring retry_after", {
            label,
            retryAfter,
            attempt,
          });
        ...
        this.log.error("telegram write failed", {
          label,
          attempt,
          ...errFields(e),
        });
  ...
  persist(text: string, extra?: Fields): Promise<Message> {
    return this.enqueue("sendMessage", () =>
      this.api.sendMessage(this.route.chatId, text, {
        ...this.thread,
        ...(extra as object),
      }),
    );
  ...
  sendPhoto(
    ...
    opts?: { caption?: string; has_spoiler?: boolean },
  ): Promise<Message> {
    return this.enqueue("sendPhoto", () =>
      this.api.sendPhoto(this.route.chatId, photo, { ...this.thread, ...opts }),
    );
  }

  sendDocument(
    doc: InputFile | string,
    opts?: { caption?: string },
  ): Promise<Message> {
    return this.enqueue("sendDocument", () =>
      this.api.sendDocument(this.route.chatId, doc, {
        ...this.thread,
        ...opts,
      }),
    );
  ...
  sendVoice(
    ...
    opts?: { duration?: number; caption?: string },
  ): Promise<Message> {
    return this.enqueue("sendVoice", () =>
      this.api.sendVoice(this.route.chatId, voice, { ...this.thread, ...opts }),
    );
  }

  sendMediaGroup(
    media: readonly InputMediaPhoto[] | readonly InputMediaDocument[],
  ): Promise<Message[]> {
    return this.enqueue("sendMediaGroup", () =>
      this.api.sendMediaGroup(this.route.chatId, media, { ...this.thread }),
    );
  }

  sendLocation(latitude: number, longitude: number): Promise<Message> {
    return this.enqueue("sendLocation", () =>
      this.api.sendLocation(this.route.chatId, latitude, longitude, {
        ...this.thread,
      }),
    );
  }
  ...
  pin(messageId: number): Promise<true> {
    return this.enqueue("pinChatMessage", () =>
      this.api.pinChatMessage(this.route.chatId, messageId, {
  ...
  editText(messageId: number, text: string, extra?: Fields): Promise<unknown> {
    return this.enqueue("editMessageText", () =>
      this.api.editMessageText(this.route.chatId, messageId, text, {
        ...(extra as object),
      }),
    );
  }
  ...
  deleteMessage(messageId: number): Promise<true> {
    return this.enqueue("deleteMessage", () =>
      this.api.deleteMessage(this.route.chatId, messageId),
    );
  }
  ...
  react(messageId: number, emoji: string): Promise<true> {
    return this.enqueue("setMessageReaction", () =>
      this.api.setMessageReaction(this.route.chatId, messageId, [
