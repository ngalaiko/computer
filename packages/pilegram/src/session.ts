...
export class Session {
  ...
  private constructor(
    ...
    this.log.info("session ready", {
      sessionFile: this.agent.sessionFile ?? null,
    });
  }
  ...
  static async create(opts: SessionOptions): Promise<Session> {
    ...
    const extensionNames = (
      opts.resourceLoader?.getExtensions().extensions ?? []
    ...
    const onSent = opts.messageLog
      ? (id: number, text: string) => opts.messageLog!.add(id, "bot", text)
      : undefined;
  ...
  private onEvent(event: Parameters<Parameters<PiSession["subscribe"]>[0]>[0]) {
    switch (event.type) {
      ...
      case "tool_execution_start": {
        ...
        else if (!event.toolName.startsWith("tg_"))
          this.renderer.onToolStart(event.toolName, event.args);
        break;
      }
  ...
  async speak(text: string) {
    ...
    try {
      const { path, durationSec } = await this.voice.synthesize(
        clipForSpeech(text),
        join(this.workspaceDir, "tts"),
      );
      await this.writer.sendVoice(new InputFile(path), {
      ...
      await this.writer
        .persist(text)
        .catch((e2) => this.log.error("text fallback failed", errFields(e2)));
    }
  }
