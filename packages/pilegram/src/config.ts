  ...
  const ids = raw
    ...
    .map((s) => {
      ...
      if (!Number.isInteger(n) || n <= 0)
        throw new Error(`invalid user id in --allow: ${JSON.stringify(s)}`);
      return n;
    });
  if (ids.length === 0)
    throw new Error("--allow is required (comma-separated Telegram user ids)");
  return new Set(ids);
}
...
function parsePositiveInt(
  ...
  if (!Number.isInteger(n) || n <= 0)
    throw new Error(
      `invalid ${flag}: ${JSON.stringify(raw)} (expected a positive integer)`,
    );
  return n;
}
...
function xdgDir(envVar: string, homeFallback: string): string {
  ...
  return join(
    base && base !== "" ? base : join(homedir(), homeFallback),
    "pilegram",
  );
}
...
export function loadConfig(argv: string[] = process.argv.slice(2)): Config {
  ...
  if (!botToken) {
    process.stderr.write(
      `Missing TELEGRAM_BOT_TOKEN in the environment (the one secret; keep it out of flags).\n\n${USAGE}`,
    );
    process.exit(2);
  }
  ...
  const stateDir = resolve(
    str("state-dir") ?? xdgDir("XDG_CONFIG_HOME", ".config"),
  );
  const modelsDir = resolve(
    str("models-dir") ?? xdgDir("XDG_CACHE_HOME", ".cache"),
  );
